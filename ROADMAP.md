...
- [ ] 7.A.1 **Backend service architecture decision** — append to `DECISIONS.md` formalising the row-69 Supabase-as-everything lock with concrete spec: Supabase project provisioning plan (region, tier, env separation dev/prod), auth model (magic-link email per row 70), proxy latency budget (p95 forward time + Anthropic round-trip + cache_control passthrough verification), metering store schema (counters table shape), observability (cost-run-up alerting threshold; daily Anthropic spend dashboard), abuse-detection signals. Decision-only task; no implementation.
- [ ] 7.A.2 **Backend service implementation** — Supabase project + Edge Function for LLM proxy that forwards Anthropic calls with PetPal's key (must transparently passthrough Anthropic `cache_control` blocks so prompt caching still works); auth via Supabase Auth; per-user message counter (resets monthly on entitlement-renewal date) **wrapped in DB transaction with row-level lock** (risk mitigation per Stage 1) so concurrent requests at the 199/200 boundary serialize correctly; per-user vision counter; photo-credit balance; hardening: rate-limit floor (anonymous device token + signed-in user ID), abuse detection, log retention with no chat content stored.
- [ ] 7.A.3 **`AnthropicClient` two-path refactor** — introduce `LlmTransport` abstraction at `lib/harness/agent/llm_transport.dart`. Existing direct-call code at `lib/harness/agent/anthropic_client.dart` becomes `DirectTransport` (BYOK path per row 74). New `ProxyTransport` calls the 7.A.2 backend. Agent loop and prompt-caching layer unchanged — selection happens at construction time based on active entitlement state (Free + funded → Proxy; Free + BYOK → Direct; Pro → Proxy). Tests cover both transports including `cache_control` passthrough on the proxy path.

### Group B — Tier Service & Entitlements (Medium, 1 task)

- [ ] 7.B.1 **Entitlement model + Drift schema bump** — new `entitlements` table (state ∈ {`free`, `pro_monthly`, `pro_annual`, `byok`}, renewal_date, photo_credits_balance, monthly_text_count, monthly_vision_count, counter_period_start, grace_until). Schema version 1 → 2 with additive migration. Riverpod `entitlementProvider` exposes the active state to UI + agent loop. Per row 78, Supabase is canonical for state — local entitlement table is a cache; reconcile on app foreground + on chat-screen mount + after Play webhook fires.

### Group C — Play Billing Integration (Medium, 3 tasks)

- [ ] 7.C.1 **Subscription IAPs** — `in_app_purchase` package adoption (DECISIONS row required + plugin-bump checklist per row 33). Pro $7.99/mo + $59/yr per row 36. Receipt → backend → entitlement update per row 78 (server-side Play receipt verification via `subscriptionsv2.get` is the trust anchor). All testing via Play Console tester accounts; **no real-card testing** (risk mitigation lock).
- [ ] 7.C.2 **Photo credit pack IAP** — $2.99 = 50 vision analyses, consumable IAP, balance rolls over indefinitely per row 36. Pro-only feature. Backend records the credit grant; client reads via `entitlementProvider`. Tester-account testing only.
- [ ] 7.C.3 **Care pack IAP** — one starter pack ($2.99–$4.99 range, e.g. "Reactive Dog" or "Senior Cat"). Non-consumable IAP. Wires `SkillManifest.requiresPro` enforcement at the skill-loader (the field exists at `lib/harness/skills/skill_manifest.dart:23,34,45` but is unenforced today — this task lights it up). Pack content authoring + skill-loader integration in the same task. **Expert pack ($14.99–$39.99) deferred to v1.x per DECISIONS row 81 + V1X_BACKLOG entry.**

### Group D — Quota Enforcement (Medium, 1 task)

- [ ] 7.D.1 **Quota gates wired across the stack** — pre-call gate at `lib/harness/agent/agent_loop.dart` consults `entitlementProvider` (per row 75 hybrid model). Free: 200 msg/mo, red-flag-screened turns exempt and never counted (verified by guarding the increment behind the screener result; Phase 4 fixtures cover this). Pro: unmetered text + 30 vision/mo + photo-credit balance. Free + BYOK: no quota (calls go direct to Anthropic via `DirectTransport`; PetPal's backend never sees them). Reminders gate at `lib/harness/scheduling/scheduler.dart` enforces 5-cap free / unlimited Pro per row 79. Pet count gate at `lib/data/repos/pet_repo.dart` enforces 1-pet free / unlimited Pro. Sync gate at the existing `CloudSyncAdapter` interface (`lib/data/sync/cloud_sync_adapter.dart`) checks Pro entitlement before any sync operation.

### Group E — Paywall + Pro UX (Medium, 2 tasks)

- [ ] 7.E.1 **Paywall screens + restore-purchases + 200/mo wall** — Pro upgrade screen with VOICE.md §6 example 14 + §7 additive copy ("Pro lifts the limit," not "you've hit the cap"). Hard wall on chat at 200 msg/mo per row 73; inline panel offers two CTAs: Upgrade to Pro + Use my own Anthropic key (BYOK escape valve). Restore-purchases covers subs + credit-pack history per row 78. Settings gets a quota counter row + photo-credit balance row + restore-purchases entry. **Paywall + wall + counter + restore copy surfaces during this task for review** — wall must read warm, not adversarial.
- [ ] 7.E.2 **Multi-pet UI** — pet switcher widget, cross-pet timeline, family-wide reminders. Pro-gated; free-tier add-pet block uses VOICE.md §6 example 9 copy. (Moved from the original Phase 6 candidate list per DECISIONS row 34; multi-pet UI shipped in Phase 6 = unused for non-paying users, belongs alongside the paywall.)

### Group F — BYOK Path (Small, 1 task)

- [ ] 7.F.1 **BYOK onboarding rewrite + Settings switcher** — restructure `lib/app/screens/onboarding_screen.dart`: API-key entry is no longer a required step. New onboarding flow: welcome → privacy disclosure (proxy-default copy from VOICE.md §6 example 15) → done. Settings gets a "Bring your own Anthropic key" toggle (VOICE.md §6 example 12 copy) that — per row 74 — runs format check + live ping to Anthropic `/v1/models` at toggle activation, then activates `DirectTransport` and stores the key via existing `SecureApiKeyStorage` (`lib/platform/api_key_storage.dart`). Existing keys persist on upgrade — Phase 7 doesn't require existing users to re-enter their key.
