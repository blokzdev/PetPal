...
- [x] 6.5.2 Stitch curation (Stage 2). Task list locks once Stage 1
  ships and Stitch sessions are run. **Absorbed into Phase 6.6 — see
  DECISIONS row 58 (Stitch curation + implementation rolled into the
  Phase 6.6 redesign sweep rather than shipped as 6.5.2 / 6.5.3).**
- [x] 6.5.3 Implementation (Stage 3). Task list locks once Stage 2
  curation completes. **Absorbed into Phase 6.6 — see DECISIONS row 58.**
...
- [x] 7.G.1 **Sync backend confirmation** — DECISIONS row 83 lands the operational spec for Supabase Storage as the v1 sync object store. Single bucket `wiki` per project, object key `<user_id>/<pet_id>/<relative_path>.enc`, per-user RLS via `auth.uid()::text = (storage.foldername(name))[1]`, native S3 versioning (30-version retention), defense-in-depth encryption (E2EE per row 71 + Supabase server-side AES-256), object body shape `[salt][IV][AAD: pet_id+path+write_ts][ciphertext][GCM tag]`, sidecar `wiki_sync_objects` Postgres table for cheap delta queries (`body_hash` of plaintext, `write_ts`, `deleted` flag), bandwidth model (~2 KB/entry, ~1 MB total for a 500-entry user). BYOC + photo sync filed in V1X_BACKLOG with structured deferral notes. Decision-only commit.
- [x] 7.G.2 **CloudSyncAdapter implementation + E2EE** — Argon2id passphrase-derived AES-256-GCM E2EE for synced wiki content. New `lib/data/sync/wiki_crypto.dart` (`Argon2id` KDF with row 71 starting params + `AesGcm.with256bits` body cipher; wire format `[1 byte version][12 bytes IV][ciphertext][16 bytes GCM mac]` with AAD `{pet_id, path, write_ts}` carried out-of-band so tampering the sidecar fails verification). New `lib/data/sync/sync_session.dart` (in-memory derived-key cache + `SyncChallenge` setup/unlock flows; passphrase challenge encrypts a known constant under the derived key, second device decrypts to validate). New `lib/data/sync/sync_backend.dart` abstract surface (`fetchChallenge` / `storeChallenge` / `listSince` / `downloadObject` / `uploadObject` / `markDeleted`) with `RemoteObjectMeta` + `InMemorySyncBackend` test fake. New `lib/data/sync/e2ee_sync_adapter.dart` `CloudSyncAdapter` impl wiring crypto + session + backend; push enumerates wiki files, hashes, encrypts, uploads ciphertext + sidecar in one call; pull queries sidecar for delta-since-last-sync, downloads ciphertext, decrypts under the cached key, writes locally. **The "backend never sees plaintext" invariant test** at `test/data/sync/e2ee_sync_adapter_test.dart` scrapes captured upload bytes for unique plaintext markers — fails if any byte sequence of the plaintext appears in the network payload. New `lib/app/sync/passphrase_setup_screen.dart` implements row 71's modal two-step acknowledgment lock — Step 1: "PetPal cannot recover this passphrase if I lose it" + checkbox + Continue; Step 2: "I have written this down somewhere safe" + checkbox + Continue; Step 3: passphrase entry + confirm. PopScope walks back stage-by-stage; back from step 1 cancels. New `lib/app/sync/sync_providers.dart` Riverpod graph (`syncSession`, `syncBackend`, `syncSetupAction`, `syncUiState`); G.2 ships an unauthenticated `InMemorySyncBackend` default — H.1 will swap in `SupabaseSyncBackend` alongside magic-link auth. Settings Plan card gains a `_SyncCard` with five-state UI (`proLocked` → "Compare plans"; `signedOut` → "Sign in coming"; `setupNeeded` → opens passphrase setup; `locked` → opens unlock sheet; `unlocked` → "Sync is on"). `passphrase_unlock_sheet` lets second-device users re-enter passphrase. `Entitlement.byok()` factory unchanged. **DECISIONS row 84 amends row 83**: salt is per-user (not per-object), wire format simplified — per-object Argon2id would punish first-sync wall time crushingly (~4min for a 500-entry library); GCM with random IV per encrypt is already pattern-safe. 24 new tests across `wiki_crypto_test.dart` (12 — round-trip, AAD tamper, key derivation, blob shape), `sync_session_test.dart` (6 — challenge flow, locked-state invariants), `e2ee_sync_adapter_test.dart` (6 — push/pull, the load-bearing plaintext-leak invariant, deleted-marker propagation, auth + lock gating). `cryptography: ^2.7.0` added (pure Dart, no native plugin manifest changes per row 33 N/A). **Out of scope for G.2 (all routed forward):** production `SupabaseSyncBackend` lands in H.1 with the auth session it depends on; conflict resolution (`.conflict.md`, 5s skew tolerance) lands in G.3.
- [x] 7.G.3 **Conflict resolution** — `lib/data/sync/conflict_resolver.dart` implements DECISIONS row 72's locked rules. Pure LWW outside the 5-second skew window; within the window, structural changes (frontmatter key adds/removes) emit `<original>.conflict.md` with the deterministic loser preserved, body-only edits do simple LWW. Identical-body short-circuit (local.bytes == remote.bytes → no-op regardless of ts). Defensive `.conflict.md` when local has no recorded ts (post-restart) + diverged body — preserves the user's pre-restart edit instead of silently overwriting. Cross-device determinism: exact-tie ts cases break by lexicographic comparison of SHA-256 body hash so both devices independently pick the same survivor + same loser. Wired into `E2eeSyncAdapter.pull` via the new `ConflictResolver` constructor param (defaults to `const ConflictResolver(skewTolerance: Duration(seconds: 5))`). 17 new tests across `conflict_resolver_test.dart` (LWW outside window, structural divergence, body-only LWW, identical-body short-circuit, post-restart defensive conflict, conflictPathFor naming, configurable skew tolerance, cross-device determinism) + `conflict_integration_test.dart` (the row 72 canonical scenario — offline writes on both devices, both come back online, `.conflict.md` lands deterministically; plus the body-only LWW happy path with no `.conflict.md`). `conflictPathFor()` derives `<original>.conflict.md` (strips trailing `.md`).
